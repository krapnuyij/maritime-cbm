...
4. 최초 검증 시 사용한 원본 파일의 SHA-256 해시를 이 문서의 `실제 검증 결과`에 남긴다.
...
아직 실제 원본 파일을 검증하지 않았다. M1 최초 검증 후 다음 정보를 기록한다.

- 다운로드 날짜와 공식 출처 URL
- 원본 파일별 상대경로, 크기와 SHA-256
- 실제 압축 해제 구조
- 행·열 수, 변수 순서와 결측값 검증 결과
- 운항 속도, `kMc`, `kMt` 격자 검증 결과
- 상수 열과 중복 또는 결정적 관계가 있는 입력 변수 확인 결과
