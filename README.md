...
GitHub Actions는 Ubuntu에서 lock 파일, Ruff, 포맷과 pytest를 검증한다. 최초 Linux/X64
실행에서 39개 테스트가 통과하고 원본 데이터가 필요한 통합 테스트 3개가 skip됐다. 원본
UCI 파일은 라이선스 표기 충돌과 재배포 방침 때문에 CI에서 내려받지 않는다.
...
EDA 관찰 수치는 원본이 있는 macOS와 Linux/aarch64 환경에서 별도로 검증했다.
