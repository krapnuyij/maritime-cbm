...
평균 bias와 별개로 예측 범위는 실제 test 범위보다 넓었다. 압축기 `kMc`와 터빈 `kMt`의
`prediction_span_ratio`는 각각 2.386, 2.089이고 `prediction_slope`는 0.877, 0.813이다.
즉 실제 추세보다 완만하게 반응하면서 개별 예측은 양방향으로 퍼졌다. M4 경보 정책에서는
평균적인 건강 방향 bias에 따른 누락 가능성과 임계값 주변의 오경보 가능성을 함께 평가해야
한다. 공식 임계값이 없으므로 현재 단계에서는 실제 누락률·오경보율을 주장하지 않는다.

상태 그룹 checkpoint는 전처리 상태를 포함해 46,325 byte로, M2 Random Forest joblib
253,287,541 byte보다 약 5,468배 작다. 세 checkpoint, manifest와 압축된 행 단위 예측을
포함한 M3 artifact 전체는 415,937 byte다. 후자는 구성 범위가 달라 M2 모델 파일과 직접
비교하지 않으며, 실제 배포 지연시간과 메모리는 M5에서 측정한다.
